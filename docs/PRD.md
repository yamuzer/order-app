...
- 1차 프런트엔드 구현은 백엔드 연결 전까지 React 메모리 상태로 데이터를 관리한다.
...
| 규칙 ID | 내용 |
|---------|------|
| ORDER-01 | 장바구니의 모든 항목을 주문으로 생성한다. 1차 프런트 구현은 메모리 상태에 저장하고, API 연동 시 서버에 전송한다. |
...
- [ ] 주문하기 클릭 시 주문이 생성되고, 성공 시 장바구니가 비워진다. API 연동 시에는 주문 생성 API를 호출한다.
- [ ] 빈 장바구니에서는 주문할 수 없다.
...
| 주문 접수 | 주문이 생성되어 접수된 주문 | 상태 `ACCEPTED` |
| 제조 중 | 현재 제조 진행 중인 주문 | 상태 `IN_PROGRESS` |
| 제조 완료 | 제조가 끝난 주문 | 상태 `COMPLETED` |

> 고객이 방금 넣은 **신규 주문**은 기본 상태 `ACCEPTED`로 생성되며, 총 주문과 주문 접수 건수에 포함한다.
...
**와이어프레임 예시 해석**
- 주문 1건: `7월 31일 13:00`, `아메리카노(ICE) x 1`, `4,000원`, 버튼 `제조 중`
- → 상태 `ACCEPTED`인 신규 주문으로 해석한다.
...
| AD-02 | 신규 주문 발생 | 총 주문 수와 주문 접수 수 증가, 주문 현황에 `ACCEPTED` 행 추가, 버튼 `제조 중` |
| AD-03 | 제조 중 클릭 | 상태 `IN_PROGRESS`, 대시보드 **제조 중** +1 |
| AD-04 | 완료 클릭 | 상태 `COMPLETED`, 대시보드 **제조 완료** +1 |
| AD-05 | 재고 `+` 클릭 | 해당 메뉴 재고 +1, API 연동 시 서버 반영 |
...
**상태 변경 요청 예**
- `ACCEPTED` → `IN_PROGRESS`: body `{ "status": "IN_PROGRESS" }`
...
- [ ] 대시보드에 `총 주문`, `주문 접수`, `제조 중`, `제조 완료`가 카드 형식으로 표시된다.
- [ ] 와이어프레임 3종 메뉴 재고가 카드 UI로 표시되고 `+`/`-`로 수량을 조정할 수 있다.
- [ ] 재고는 0 미만으로 내려가지 않는다.
- [ ] 주문 현황에 시각·항목 요약·금액·상태 변경 버튼이 표시된다.
- [ ] 신규 주문은 `ACCEPTED` 상태로 생성되고 `주문 접수`으로 표시된다.
- [ ] 이후 `제조 중` → `IN_PROGRESS`, `완료` → `COMPLETED`로 순차 전환된다.
...
| OI-A02 | 주문 시 재고 자동 차감 시점 | 제조 완료 시 차감으로 결정 (§5.4.4 INV-05) |
...
| 항목 | 내용 |
|------|------|
| 범위 | 커피 메뉴 조회, 옵션 조회, 주문 생성, 주문 조회, 주문 상태 변경, 재고 차감 |
...
프런트엔드 주문하기 화면과 관리자 화면이 공통 데이터베이스를 기준으로 동작하도록 API와 데이터 스키마를 제공한다.
...
커피 메뉴 정보를 저장한다. 주문하기 화면에는 메뉴명, 설명, 가격, 이미지가 표시되고, 관리자 화면에는 재고 수량이 표시된다.

| 컬럼 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `id` | UUID 또는 bigint | O | 메뉴 고유 ID |
| `name` | varchar | O | 커피 이름 |
| `description` | text | O | 메뉴 설명 |
| `price` | integer | O | 기본 가격, 원 단위 |
| `image_url` | text | O | 메뉴 이미지 경로 또는 URL |
| `stock_quantity` | integer | O | 현재 재고 수량 |
...
- 프런트엔드 주문하기 화면에서는 `stock_quantity`를 직접 노출하지 않고, 관리자 화면의 재고 현황에서 표시한다.

#### 6.2.2 options

메뉴에 연결되는 선택 옵션 정보를 저장한다.

| 컬럼 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `id` | UUID 또는 bigint | O | 옵션 고유 ID |
| `menu_id` | UUID 또는 bigint | O | 연결할 메뉴 ID |
| `name` | varchar | O | 옵션 이름 |
| `price` | integer | O | 옵션 추가 가격, 원 단위 |
| `created_at` | timestamp | O | 생성 일시 |
| `updated_at` | timestamp | O | 수정 일시 |

**관계**
- `options.menu_id`는 `menus.id`를 참조한다.
- 한 메뉴는 여러 옵션을 가질 수 있다.
- 옵션은 연결된 메뉴에서만 선택할 수 있다.

#### 6.2.3 orders

주문 일시, 주문 상태, 주문 총액을 저장한다.

| 컬럼 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `id` | UUID 또는 bigint | O | 주문 고유 ID |
| `ordered_at` | timestamp | O | 주문 일시 |
| `status` | varchar | O | 주문 상태 |
| `total_amount` | integer | O | 주문 총액 |
| `created_at` | timestamp | O | 생성 일시 |
| `updated_at` | timestamp | O | 수정 일시 |
...
| 상태 | 표시명 | 설명 |
|------|--------|------|
| `ACCEPTED` | 주문 접수 | 주문이 접수된 기본 상태 |
| `IN_PROGRESS` | 제조 중 | 음료 제조 중 |
| `COMPLETED` | 완료 | 제조 완료 |

**규칙**
- 주문 생성 시 기본 상태는 `ACCEPTED`이다.
- 상태는 `ACCEPTED` → `IN_PROGRESS` → `COMPLETED` 순서로만 변경할 수 있다.
- 주문 총액은 서버에서 메뉴 가격과 옵션 가격을 기준으로 다시 계산한다.

#### 6.2.4 order_items

주문에 포함된 메뉴, 수량, 금액 스냅샷을 저장한다.

| 컬럼 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `id` | UUID 또는 bigint | O | 주문 항목 고유 ID |
| `order_id` | UUID 또는 bigint | O | 주문 ID |
| `menu_id` | UUID 또는 bigint | O | 메뉴 ID |
| `menu_name` | varchar | O | 주문 당시 메뉴명 스냅샷 |
| `quantity` | integer | O | 주문 수량 |
| `unit_price` | integer | O | 옵션 포함 단가 |
| `line_amount` | integer | O | 항목 금액, `unit_price * quantity` |

**규칙**
- `order_items.order_id`는 `orders.id`를 참조한다.
- `order_items.menu_id`는 `menus.id`를 참조한다.
- `quantity`는 1 이상이어야 한다.
- 메뉴명과 가격은 주문 당시 값을 스냅샷으로 저장한다.

#### 6.2.5 order_item_options

주문 항목에 선택된 옵션 스냅샷을 저장한다.

| 컬럼 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `id` | UUID 또는 bigint | O | 주문 옵션 고유 ID |
| `order_item_id` | UUID 또는 bigint | O | 주문 항목 ID |
| `option_id` | UUID 또는 bigint | O | 옵션 ID |
| `option_name` | varchar | O | 주문 당시 옵션명 스냅샷 |
| `option_price` | integer | O | 주문 당시 옵션 가격 스냅샷 |

**규칙**
- `order_item_options.order_item_id`는 `order_items.id`를 참조한다.
- 옵션명과 가격은 주문 당시 값을 스냅샷으로 저장한다.

### 6.3 데이터 스키마를 위한 사용자 흐름

#### 6.3.1 메뉴 조회 흐름

1. 사용자가 브라우저에서 `주문하기` 메뉴를 클릭한다.
2. 프런트엔드는 백엔드에 커피 메뉴 목록을 요청한다.
3. 백엔드는 `menus`와 연결된 `options`를 조회한다.
4. 브라우저는 `menus.name`, `menus.description`, `menus.price`, `menus.image_url`, `options`를 주문하기 화면에 표시한다.
5. `menus.stock_quantity`는 주문하기 화면에는 표시하지 않고 관리자 화면의 재고 현황에 표시한다.

#### 6.3.2 장바구니 담기 흐름

1. 사용자가 앱 화면에서 커피 메뉴를 선택한다.
2. 사용자가 옵션을 선택한다.
3. 사용자가 `담기` 버튼을 클릭한다.
4. 선택한 메뉴, 수량, 옵션, 금액 정보가 브라우저의 장바구니에 표시된다.
5. 동일 메뉴와 동일 옵션 조합은 수량을 증가시키고, 옵션 조합이 다르면 별도 항목으로 표시한다.

#### 6.3.3 주문 생성 흐름

1. 사용자가 장바구니에서 `주문하기` 버튼을 클릭한다.
2. 프런트엔드는 주문 항목의 `menu_id`, `quantity`, `option_ids`를 백엔드에 전송한다.
3. 백엔드는 메뉴와 옵션 정보를 데이터베이스에서 다시 조회한다.
4. 백엔드는 주문 금액을 서버 기준으로 재계산한다.
5. 백엔드는 `orders`에 주문 일시, 주문 상태, 총액을 저장한다.
6. 백엔드는 `order_items`에 메뉴, 수량, 단가, 항목 금액을 저장한다.
7. 백엔드는 `order_item_options`에 선택 옵션을 저장한다.
8. 백엔드는 주문 정보에 따라 `menus.stock_quantity`를 주문 수량만큼 차감한다.
9. 백엔드는 생성된 주문 ID와 주문 상세 정보를 응답한다.

#### 6.3.4 관리자 주문 현황 흐름

1. 관리자 화면은 `orders`에 저장된 주문 정보를 조회한다.
2. 백엔드는 주문 일시, 주문 상태, 주문 항목, 옵션, 수량, 금액을 함께 반환한다.
3. 관리자 화면의 `주문 현황`에는 최신 주문이 위쪽에 표시된다.
4. 주문의 기본 상태는 `주문 접수`이다.
5. 관리자가 상태 버튼을 클릭하면 주문 상태가 `제조 중`으로 변경된다.
6. 관리자가 다시 상태 버튼을 클릭하면 주문 상태가 `완료`로 변경된다.

### 6.4 API 설계

#### 6.4.1 커피 메뉴 목록 조회

`주문하기` 메뉴를 클릭하면 데이터베이스에서 커피 메뉴 목록을 불러와 브라우저에 표시한다.

| 항목 | 내용 |
|------|------|
| Method | `GET` |
| Path | `/api/menus` |
| 설명 | 판매 중인 커피 메뉴와 메뉴별 옵션을 조회한다. |
...
**처리 규칙**
- `menus`와 `options`를 조인하여 반환한다.
- 주문하기 화면은 응답 중 `stockQuantity`를 사용하지 않을 수 있다.
- 관리자 화면은 `stockQuantity`를 재고 현황에 표시한다.

#### 6.4.2 주문 생성

사용자가 커피를 선택하고 `주문하기` 버튼을 누르면 주문 정보를 데이터베이스에 저장한다. 주문 정보에 따라 메뉴 재고도 수정한다.

| 항목 | 내용 |
|------|------|
| Method | `POST` |
| Path | `/api/orders` |
| 설명 | 장바구니 정보를 기반으로 주문을 생성하고 재고를 차감한다. |
...
```json
{
  "order": {
    "id": "order-1",
    "orderedAt": "2026-05-16T09:00:00.000Z",
    "status": "ACCEPTED",
    "statusLabel": "주문 접수",
    "totalAmount": 9000,
    "items": [
...
**처리 규칙**
- 서버는 클라이언트가 보낸 금액을 신뢰하지 않는다.
- 서버는 `menus.price`와 `options.price`로 주문 금액을 계산한다.
- 선택한 옵션이 해당 메뉴에 연결된 옵션인지 검증한다.
- 재고가 부족하면 주문을 생성하지 않고 오류를 반환한다.
...
| 상태 코드 | 상황 |
|-----------|------|
| `400` | 요청 형식 오류, 수량 1 미만, 잘못된 옵션 |
| `404` | 존재하지 않는 메뉴 또는 옵션 |
| `409` | 재고 부족 |
| `500` | 서버 오류 |

#### 6.4.3 주문 목록 조회

관리자 화면의 `주문 현황`에 표시할 주문 목록을 조회한다.

| 항목 | 내용 |
|------|------|
| Method | `GET` |
| Path | `/api/orders` |
| 설명 | 주문 목록을 최신순으로 조회한다. |
...
주문 ID를 전달하면 해당 주문 정보를 보여준다.

| 항목 | 내용 |
|------|------|
| Method | `GET` |
| Path | `/api/orders/:orderId` |
| 설명 | 주문 ID에 해당하는 주문 상세 정보를 조회한다. |
...
| 상태 코드 | 상황 |
|-----------|------|
| `404` | 주문 ID에 해당하는 주문이 없음 |

#### 6.4.5 주문 상태 변경

관리자 화면에서 주문 상태 버튼을 클릭하면 주문 상태를 다음 단계로 변경한다.

| 항목 | 내용 |
|------|------|
| Method | `PATCH` |
| Path | `/api/orders/:orderId/status` |
| 설명 | 주문 상태를 `ACCEPTED` → `IN_PROGRESS` → `COMPLETED` 순서로 변경한다. |
...
**처리 규칙**
- 현재 상태의 다음 단계만 허용한다.
- `COMPLETED` 상태의 주문은 더 이상 변경할 수 없다.
- 허용되지 않은 상태 전이는 `400`을 반환한다.

#### 6.4.6 관리자 재고 조회

관리자 화면의 `재고 현황`에 표시할 메뉴별 재고를 조회한다.

| 항목 | 내용 |
|------|------|
| Method | `GET` |
| Path | `/api/admin/inventory` |
| 설명 | 메뉴별 현재 재고 수량을 조회한다. |
...
관리자가 `+` 또는 `-` 버튼으로 재고를 조정한다.

| 항목 | 내용 |
|------|------|
| Method | `PATCH` |
| Path | `/api/admin/inventory/:menuId` |
| 설명 | 특정 메뉴의 재고를 증감한다. |
...
| 규칙 ID | 내용 |
|---------|------|
| DB-01 | 주문 생성과 재고 차감은 하나의 트랜잭션으로 처리한다. |
| DB-02 | 주문 생성 시 메뉴 가격과 옵션 가격은 서버에서 조회하고 재계산한다. |
| DB-03 | 주문 항목에는 메뉴명, 옵션명, 가격을 스냅샷으로 저장한다. |
| DB-04 | 재고 부족 시 주문과 재고 변경을 모두 롤백한다. |
| DB-05 | 주문 목록과 주문 상세 조회는 주문 항목 및 옵션 정보를 함께 반환한다. |

### 6.6 백엔드 수용 기준

- [ ] `GET /api/menus` 호출 시 데이터베이스의 메뉴와 옵션이 반환된다.
- [ ] 주문하기 화면은 메뉴 목록을 API 응답으로 표시할 수 있다.
- [ ] 장바구니에서 `주문하기` 클릭 시 `POST /api/orders`로 주문이 저장된다.
- [ ] 주문 생성 시 주문 시간, 주문 내용, 메뉴, 수량, 옵션, 금액이 저장된다.
- [ ] 주문 생성 시 메뉴 재고가 주문 수량만큼 차감된다.
- [ ] 재고 부족 시 주문이 저장되지 않고 오류가 반환된다.
- [ ] 관리자 화면은 `GET /api/orders`로 주문 현황을 표시할 수 있다.
- [ ] 주문 기본 상태는 `ACCEPTED`이며 화면에는 `주문 접수`로 표시된다.
- [ ] 주문 상태는 `주문 접수` → `제조 중` → `완료` 순서로 변경된다.
- [ ] 주문 ID를 전달하면 해당 주문 상세 정보를 조회할 수 있다.
